...
@Configuration
@MapperScan(basePackages = "com.rivergame.fvgm.dao.farmplayers", sqlSessionTemplateRef = "farmPlayersSqlSessionTemplate")
public class DataSource3Config {
    @Bean(name = "farmPlayersDataSource")
    public DruidDataSource dataSource() {
        String prefix = "datasource_farm_players_";
    ...
    @Bean(name = "farmPlayersTransactionManager")
    ...
    @Bean(name = "farmPlayersSqlSessionFactory")
    public SqlSessionFactoryBean mybatisSqlSessionFactory() {
        SqlSessionFactoryBean sqlSessionFactoryBean = new SqlSessionFactoryBean();
        sqlSessionFactoryBean.setDataSource(dataSource());
        PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
        try {
            sqlSessionFactoryBean.setMapperLocations(resolver.getResources("classpath:/mapper/farmplayers/*.xml"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @Bean(name = "farmPlayersSqlSessionTemplate")
